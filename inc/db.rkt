...
(define (sentences-with-literal literal)
  (unless (connected? connection) (set! connection (connect)))
  (vector-ref (query-row connection
                         "SELECT sentence FROM literals WHERE literal = ? ORDER BY RAND() LIMIT 1;"
                         literal)
              0))
...
(define (sentences-with-ratio ratio)
  (unless (connected? connection) (set! connection (connect)))
  (vector-ref (query-row connection
                         "SELECT sentence FROM ratios WHERE top = ? AND bot = ? ORDER BY RAND() LIMIT 1;"
                         (ratio-top ratio)
                         (ratio-bot ratio))
              0))
...
(define (sentences-matching-no-ratio)
  (unless (connected? connection) (set! connection (connect)))
  (vector-ref (query-row connection
                         "SELECT sentence FROM ratios WHERE NOT ((top = ? AND bot = ?) OR (top = ? AND bot = ?) OR (top = ? AND bot = ?) OR (top = ? AND bot = ?) OR (top = ? AND bot = ?) OR (top = ? AND bot = ?) OR (top = ? AND bot = ?) OR (top = ? AND bot = ?) OR (top = ? AND bot = ?) OR (top = ? AND bot = ?) OR (top = ? AND bot = ?) OR (top = ? AND bot = ?) OR (top = ? AND bot = ?) OR (top = ? AND bot = ?) OR (top = ? AND bot = ?) OR (top = ? AND bot = ?) OR (top = ? AND bot = ?) OR (top = ? AND bot = ?) OR (top = ? AND bot = ?) OR (top = ? AND bot = ?) OR (top = ? AND bot = ?) OR (top = ? AND bot = ?) OR (top = ? AND bot = ?)) ORDER BY RAND() LIMIT 1;"
                         13 7 
                         2 3 
                         0 1 
                         2 1 
